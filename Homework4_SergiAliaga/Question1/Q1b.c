...
int main(int argc, char *argv[])
{
	...
	MPI_Recv(&count, 1, MPI_INT, (rank+size-1)%size, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	count--;
	printf("Process %d from the Node %s decrements count from %d to %d.\n",rank, processor_name, count+1, count);
	if(rank!=size-1) MPI_Send(&count, 1, MPI_INT, rank+1, 0, MPI_COMM_WORLD);
    }
