//Benchmark obtained from https://github.com/snehalhs/Benchmarking/blob/master/CPUBenchmark.c
...
void *threadFunctionFlopCal(void *arg)
{
	...
	for(i = 0; i < ITERATION; i++)
    	{
        	a =0.5+2+3+5+6+3+1+1+7+5+1+2+2+3+6+8+1+2+9+1;
    	}
	return NULL;
}
...
{       //menu driven program
    ...
    printf("\nProgram to find GFLOPS for %d threads\n",numberOfThreads);
    printf("Number of FLOPs (GFLOPS):\n");\
